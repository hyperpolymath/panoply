...
find "$TEST_REPO_PATH" -type f \
    ...
    -exec bash -c '
        file="$1"
        placeholder_pairs=(
            "panoply|$TEST_REPO_NAME"
            "hyperpolymath|$TEST_OWNER"
            "github.com|$TEST_FORGE"
            "PANOPLY|$TEST_PROJECT_NAME"
            "panoply|'"${TEST_REPO_NAME//-/_}"'"
            "An envelope-first language discipline: every safety claim explicit, scoped, inspectable, and mechanically accountable.|$TEST_DESCRIPTION"
            "Idris2|$TEST_PRIMARY_LANGUAGE"
            "Jonathan D.A. Jewell|$TEST_AUTHOR"
            "j.d.a.jewell@open.ac.uk|$TEST_AUTHOR_EMAIL"
            "2026-06-21|2026-04-04"
        )
