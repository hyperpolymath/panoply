...
* Copy configuration templates (Justfile, STATE.deed, etc.)
...
* **STATE.deed management**: `just state-touch`, `just state-phase`
...
│   ├── STATE.deed             # Project state, progress, blockers
│   ├── META.deed              # Architecture decisions, governance
│   ├── ECOSYSTEM.deed         # Ecosystem position, relationships
│   ├── AGENTIC.deed           # AI agent interaction patterns
│   ├── NEUROSYM.deed          # Neurosymbolic integration config
│   ├── PLAYBOOK.deed          # Operational runbook
...
└── AI.deed                    # Claude-specific instructions
----
...
* `build/guix.scm` and `build/.guix-channel`
...
The STATE.deed file tracks project state:
...
Generate badges from STATE.deed:
...
* **STATE.deed Ecosystem**: Conversation checkpoints
