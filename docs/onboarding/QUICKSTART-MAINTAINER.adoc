...
// Replace rsr-template-repo, panoply, {{DEPS}} with actuals
...
| `$PREFIX/share/panoply/`
| Data files, assets

| `$PREFIX/share/doc/panoply/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/panoply/config.toml`

Fallback: `$HOME/.config/panoply/config.toml`
...
Or via OPSM: `opsm update panoply`
...
[source,bash]
----
just install --prefix=/opt/panoply-instance1 --config=/etc/panoply/instance1.toml
just install --prefix=/opt/panoply-instance2 --config=/etc/panoply/instance2.toml
----
