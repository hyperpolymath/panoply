...
│       └── include/
│           └── panoply.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── panoply.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/panoply.h
```
...
int main() {
    void* handle = panoply_init();
    if (!handle) return 1;

    int result = panoply_process(handle, 42);
    if (result != 0) {
        const char* err = panoply_last_error();
...
Compile with:
```bash
gcc -o example example.c -lpanoply -L./zig-out/lib
```

### From Idris2

```idris
import PANOPLY.ABI.Foreign
...
#[link(name = "panoply")]
extern "C" {
    fn panoply_init() -> *mut std::ffi::c_void;
    fn panoply_free(handle: *mut std::ffi::c_void);
    fn panoply_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = panoply_init();
        assert!(!handle.is_null());

        let result = panoply_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libpanoply = "libpanoply"

function init()
    handle = ccall((:panoply_init, libpanoply), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:panoply_process, libpanoply), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:panoply_free, libpanoply), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/panoply.h
   ```
