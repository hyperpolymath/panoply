...
# ---------------------------------------------------------------------------
# Validator: check a single .deed or .deed file
# ---------------------------------------------------------------------------
validate_a2ml() {
    local file="$1"
    ...
    # AI manifest files (0-AI-MANIFEST.deed, 0.1-AI-MANIFEST.deed, etc.)
    # use markdown-style headers and free text, so identity check is relaxed
    local basename
    basename="$(basename "$file")"
    local is_manifest=false
    # `.deed` ONLY. The exemption exists because AI manifests are markdown-ish
    ...
    if [[ "$basename" == *"AI-MANIFEST"*.deed ]]; then
        is_manifest=true
    fi
    ...
    # files in the same directory (ECOSYSTEM.deed, STATE.deed) DO carry their
    # own $name/project and continue to be validated normally.
    case "$basename" in
        AGENTIC.deed|META.deed|NEUROSYM.deed|PLAYBOOK.deed|AI.deed)
            # AI.deed = free-text "AI Assistant Instructions" manifest, the same
            # doc type as 0-AI-MANIFEST.deed but with the bare name; identity is
    ...
    # TOML `key = value`. Trustfile.deed, Intentfile.deed, Mustfile.deed,
    # Adjustfile.deed etc. are policy / trust / intent / abstract files
    ...
    # scoped to legacy *.deed, which is no longer authored. (CodeRabbit, PR review.)
...
# ---------------------------------------------------------------------------
# Main: discover and validate .deed and .deed files
# ---------------------------------------------------------------------------

echo "::group::A2ML Manifest Validation"
echo "Scanning ${SCAN_PATH} for .deed and .deed files..."
echo ""

# Find all .deed and .deed files, excluding .git directory
mapfile -t a2ml_candidates < <(find "$SCAN_PATH" \( -name '*.deed' -o -name '*.deed' \) -not -path '*/.git/*' -type f | sort)
...
if [[ ${#a2ml_files[@]} -eq 0 ]]; then
    echo "::notice::No .deed or .deed files found in ${SCAN_PATH}"
...
echo "Found ${#a2ml_files[@]} .deed/.deed file(s)"
echo ""
